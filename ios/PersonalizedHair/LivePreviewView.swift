    ...
    private func apply(_ prepared: (LivePreviewPackage, CompiledHairMesh), observedFace: CanonicalObservedSurface? = nil) {
            ...
            for batch in mesh.batches {
                ...
                if let material = mesh.materials.first(where: { $0.id == batch.materialID }) {
                    ...
                    geometry.firstMaterial?.roughness.contents = material.roughness
                    geometry.firstMaterial?.lightingModel = .physicallyBased
                }
                if prepared.0.meshSettings?.radiusScale == LiveMeshSettings.modelReview.radiusScale {
                    geometry.firstMaterial?.diffuse.contents = batch.region == .fringe ? UIColor(Theme.accent) : UIColor(red: 0.18, green: 0.11, blue: 0.075, alpha: 1)
