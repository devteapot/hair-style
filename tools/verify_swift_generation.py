#!/usr/bin/env python3
"""One local Swift/API/Metal generation round trip, with automatic server cleanup."""
import argparse
import json
...
def main():
    ...
    client=subprocess.Popen([str(ROOT/'.build/debug/processing-probe'),f'http://127.0.0.1:{server.server_port}',str(out/'swift-report.json'),'--research'],
    ...
    try:
        ...
        if client.returncode!=0:raise RuntimeError('Swift generation probe failed: '+stderr[-1000:])
        ...
        if len(jobs)!=1 or jobs[0]['attempt_count']!=1:raise RuntimeError('Unexpected duplicate/retried generation')
        report=dict(method='supervised_swift_generation_roundtrip_v1',seconds=time.monotonic()-started,
